...
#include <cmath>
/*
 * Propagate one photon through Kerr spacetime.
 *
 * The photon state is passed in through:
 *
 *   posVec = (t, r, theta, phi)
 *   momVec = p^mu
 *
 * The routine advances the photon along its geodesic using a fourth-order
 * Runge-Kutta stepper. It stops when the photon:
 *
 *   - reaches the disk surface,
 *   - escapes beyond rLimitHigh,
 *   - falls below rLimitLow, near the event horizon,
 *   - or exceeds maxStep integration steps.
 *
 * The disk surface is checked using:
 *
 *   z_photon = r cos(theta)
 *   z_disk   = scaleHeightFnct(r, theta)
 *
 * so the photon is still above the disk while:
 *
 *   r cos(theta) > z_disk
 *
 * On return, posVec and momVec are overwritten with the final photon position
 * and approximate final momentum.
 */
