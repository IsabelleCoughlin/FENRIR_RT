...
if not existBool:
	for i in range(len(inputFileList)):
		...
		# BELLA: Changed metadata to radians
		deltaColumn = fits.Column(name = 'del{}'.format(heightIndex), format = '{}E'.format(nRadBins), unit = 'rad', array = [delta])
		deltaIncColumn = fits.Column(name = 'del_inc{}'.format(heightIndex), format = '{}E'.format(nRadBins), unit = 'rad', array = [deltaInc])
	...
	for i in range(len(inputFileList)):
		...
		deltaColumn = fits.Column(name = 'del1', format = '{}E'.format(nRadBins), unit = 'rad', array = currentDelta)
		deltaIncColumn = fits.Column(name = 'del_inc1', format = '{}E'.format(nRadBins), unit = 'rad', array = currentDeltaInc)
		cols = fits.ColDefs([radiusColumn,fluxColumn,deltaColumn,deltaIncColumn])
