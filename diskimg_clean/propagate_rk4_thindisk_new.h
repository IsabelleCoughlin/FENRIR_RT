...
/*
 * Propagate a photon backward from the observer through Kerr spacetime.
 *
 * The photon position is represented in Boyer-Lindquist coordinates:
 *
 *     posVec = (t, r, theta, phi).
 *
 *  The integration stops when one of the following occurs:
 *
 *   1. The photon reaches the prescribed disk photosphere: z = r cos(theta) <= scaleHeightFnct(r, theta).
 *   2. The photon reaches the near-horizon radial cutoff:  r <= rLimit.
 *   3. The number of integration steps reaches maxStep.
 * 
 *  Parameters:
 *
 *   posVec           Input/output photon position (t, r, theta, phi).
 *   momVec           Output contravariant photon tangent vector.
 *   dStep            Affine-parameter step; recalculated internally.
 *   tolerance        Step-refinement control; larger means finer steps.
 *   maxStep          Maximum permitted number of integration steps.
 *   rLimit           Near-horizon radial stopping coordinate.
 *   rEvent           Outer event-horizon radius.
 *   scaleHeightValue Legacy parameter; currently unused.
 *   rProjected       Legacy parameter; currently unused.
 */
